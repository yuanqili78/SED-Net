...
from primitive_forward_v2 import forward_pass_open_spline, forward_closed_splines, initialize_open_spline_model, \
...
class MyFittingModule:
    def __init__(self, sample_ratio,
                 closed_splinenet_path="splinenet/ckpt/train_closed_spline_1_700_0.9_bt_36_lr_0.001_trsz_28000_tsz_3000_wght_0.9.pth",
    ...
    def forward_pass_plane(self, points, normals, weights, ids, sample_points=False):
        axis, distance = self.fitting.fit_plane_torch(
            ...
            weights=weights,
            ids=ids,
            filter_ratio=self.sample_ratio
        )
        self.fitting.parameters[ids] = ["plane", axis.reshape((3, 1)), distance]
        if sample_points:
            # Project points on the surface
    ...
    def forward_pass_cylinder(self, points, normals, weights, ids, sample_points=False, RANSAC=False):
        if RANSAC:
            cylin = pyrsc.Cylinder()
            center, a, radius, inliers = cylin.fit(points.cpu().numpy(), thresh=0.1)
            print(a, center, radius)

            self.fitting.parameters[ids] = ["cylinder", a, center, radius]
            if sample_points:

                new_points, new_normals = self.fitting.sample_cylinder_trim(radius, center, a, points.cpu().numpy(), N=10000)
                # new_points = project_to_point_cloud(points, new_points)
                return new_points
            else:
                return None
            ...
            print(ids, "cylinder", a, center, radius)
            self.fitting.parameters[ids] = ["cylinder", a, center, radius]
            if sample_points:
                new_points, new_normals = self.fitting.sample_cylinder_trim(
                    radius.item(),
