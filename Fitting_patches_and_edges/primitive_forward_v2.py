...
# TODO
# pipeline使用的方法
def my_fit_one_shape(data, fitter, num_thresh=10, bad_insts=None, fn_id="0", save_para=False, spline_num_th=100):
    """
    ...
    for part_index, d in enumerate(data):
        ...
        print("current fitting id: ", part_index)
        # visualize_point_cloud(points, viz=True)
        ...
        if points.shape[0] < num_thresh or (bad_insts is not None and part_index in bad_insts):
            ...
            if points.shape[0] < spline_num_th:
            ...
            print("cloesd spline", recon_points.shape)  # 930 x 3
            ...
            open3d.io.write_triangle_mesh(
                "./new_datas/pred_closeds/pred_closed_{}_{}.ply".format(
                    fn_id, part_index
                ),
                pred_mesh,
            )
            ...
            recon_points = fitter.forward_pass_open_spline(points, weights=weight, ids=part_index,
                                                           if_optimize=if_optimize)
            recon_points = recon_points[0].cpu().numpy()
            print("open spline", recon_points.shape)  # 900 x 3
            temp = recon_points.reshape((30, 30, 3))
            pred_mesh = tessalate_points(temp, 30, 30)
            pred_mesh.paint_uniform_color([1, 0.0, 0])
            open3d.io.write_triangle_mesh(
                "./new_datas/pred_opens/pred_open_{}_{}.ply".format(
                    fn_id, part_index
                ),
                pred_mesh,
            )
...
映射types
"""
def project_types(types):
    cylinders = types == 4
    open_splines = (types == 2) | (types == 8)
    spheres = types == 5
    closed_splines = (types == 0) | (types == 9) | (types == 6) | (types == 7)
    types[closed_splines] = 0
    types[cylinders] = 2
    types[spheres] = 4
    types[open_splines] = 5
    return types
...
if __name__ == "__main__":
    ...
    spline_num_th = 20  # default = 100
    nn_num_thresh = 2

    num_thresh = 40 # 80 # 40 # 10
    NN_STRICT = True

    fliter_bad_points = True  # False
    ...
    corner_dis_thresh = 0.01 # 0.03 for 165  # ================== threshold

    if not is_gt:
        raw_points = np.loadtxt("./{}/{}_GT_points.txt".format(_dir, id), delimiter=";", usecols=np.arange(6))
        primitives = np.loadtxt("./{}/{}_inst.txt".format(_dir, id)).astype(np.int32)
        types = project_types(np.loadtxt("./{}/{}_type.txt".format(_dir, id)).astype(np.int32))
        ...
        raw_points = np.loadtxt("C:/Users/Sean/Desktop/edge_log/gt/{}_GT_points.txt".format(_dir, id), delimiter=";")
    ...
    strict_edge_idx = get_edges_between_insts(raw_points, primitives, strict=NN_STRICT).cpu().numpy()  # 得到inst之间的边界点索引
    ...
    for inst in primitive_ids:
        ...
        if labels in [0, 5]:
            _total = count.sum()
            labels_sort = np.argsort(count)[::-1]
            if labels_sort.shape[0] > 1 and labels_sort[1] in [2, 3] and float(count[labels_sort[1]]) / _total > 0.25:
                print(f"inst {inst} 's type is cone or cylinder prior to spline !!!! ")
                labels = labels_sort[1]
            elif labels_sort.shape[0] > 1 and labels_sort[1] in [1, ] and float(count[labels_sort[1]]) / _total > 0.1:
                print(f"inst {inst} 's type is plane prior to spline !!!! ")
                labels = labels_sort[1]

            elif id in ["925", "949", "1011", ]:
                labels = 2
            ...
            elif id in ["4146", "4147"]:
                labels = 3 # cone

        elif labels == 2:  # cylinder
            # index = index & ~is_edge  # 
    ...
    gt_points, reconstructed_shape = my_fit_one_shape(inst_data, fitter, num_thresh, bad_insts,
                                                      fn_id=id, spline_num_th=spline_num_th)

    with open(os.path.join(save_dir, "paras", "param_{}.txt".format(id)), "w") as para_f:
        for key in fitter.fitting.parameters.keys():
            try:
                s = "id {}: ".format(key)
                if fitter.fitting.parameters[key] is None:
                    continue
    ...
    # calculate intersection of an instance & an instance
    gen_edges = np.ones((0, 3), dtype=np.float64)
    for i, data in enumerate(reconstructed_shape):
        if fitter.fitting.parameters[i] is None:
            print("skip spline face inter line")
            None_instances.append(int(primitive_ids[i]))
            continue
        ...
        for _id in range(inter_faces.shape[0]):
            try:
                ...
                if para1[0] == para2[0] == 'plane':
                    ...
                    if id in ["925"] and inst_id_1 in [5] and inst_id_2 == 10:
                        gen_paras = plane_cylinder_inter_line(para1, para2, plane_sample=reconstructed_shape[inst_id_2][0].reshape(3,))
                    elif id in ["925"] and inst_id_1 in [11] and inst_id_2 == 10:
                        gen_paras = plane_cylinder_inter_line(para1, para2, plane_sample=reconstructed_shape[inst_id_2][-1].reshape(3,))
                    else:
    ...
    for i, inst in enumerate(primitive_ids):
        ...
        if len(neib_insts) < 3:
            # if an instance is intersected with one or two instances，it is a circle or an ellipse.
                ...
                for n in range(m + 1, len(neib_insts)):
                    ...
                    if inter_para_set[inst][neib_insts[m]][0] == inter_para_set[inst][neib_insts[n]][0] == "line":
                        ...
                        if _point is not None:
                            if fitter_point(_point, (inst_points, inst_points_m, inst_points_n), thresh=corner_dis_thresh):
    ...
    for inst_1 in range(primitive_ids.shape[0] - 1):
        for inst_2 in range(int(inst_1 + 1), int(primitive_ids.shape[0]), 1):
            ...
            if inst_1 in None_instances or inst_2 in None_instances:
                print("skip splines!")
                continue
            ...
            if intermap[primitive_ids[inst_1], primitive_ids[inst_2]]:
                ...
                if inter_para_set[inst1][inst2][0] == "line":
                    ...
                    inter_para_set[inst1][inst2].append(sorted(range_d))
                    inter_para_set[inst2][inst1].append(sorted(range_d))
                    ...
                    if isinstance(corners[0], int):
                        ...
                        inter_para_set[inst1][inst2].append([0, 2 * np.pi])
                        inter_para_set[inst2][inst1].append([0, 2 * np.pi])
                        ...
                        inter_para_set[inst1][inst2].append([angle1, angle2])
                        inter_para_set[inst2][inst1].append([angle1, angle2])
                        angles = np.arange(angle1, angle2, 0.01).reshape((-1, 1))
                        points = radius * (np.cos(angles) * x_axis + np.sin(angles) * y_axis) + center
                        visualize_point_cloud(points, viz=True)
                        final_gen_edges = np.concatenate((final_gen_edges, points), axis=0)

    with open(os.path.join(save_dir, "paras", "param_inter_lines_{}.json".format(id)), "w") as para_f:
        for k, v in inter_para_set.items():
            for k1, v1 in inter_para_set[k].items():
                for i in range(len(v1)):
                    if isinstance(v1[i], np.ndarray):
                        inter_para_set[k][k1][i] = inter_para_set[k][k1][i].tolist()
        json.dump(inter_para_set, para_f)

    with open(os.path.join(save_dir, "paras", "param_inter_lines_{}.txt".format(id)), "w") as para_f:
        for key in inter_para_set.keys():
            try:
                if inter_para_set[key] is None: continue

                
                for iter_face in inter_para_set[key].keys():
                    s = "\t"
                    s += "id {}, para: ".format(iter_face)
                    tmp = inter_para_set[key][iter_face]
                    s += "{}".format(tmp)
