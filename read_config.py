...
class Config(object):
    def __init__(self, filename: string):
        ...
        self.pretrain_model_type_path = config["train"]["pretrain_model_type_path"]
        self.preload_model = config["train"].as_bool("preload_model")
        self.pretrain_opti_path = config["train"]["pretrain_opti_path"]

        # Normals
        self.normals = config["train"].as_bool("normals")

        self.smooth = config["train"].as_float("smooth")
        ...
        self.embed = config["train"].as_int("embed")
        ...
        self.sche = config["train"]["sche"]

        self.input_drop = config["train"].as_float("encoder_drop")
        ...
        try:
            self.knn = config["train"].as_int("knn")  # ============ default 64
        except:
            self.knn = 64
            print("config no knn! use default 64")
