...
def rotate_point_cloud(batch_data):
    ...
    if not isinstance(batch_data, list):
        ...
        for i in range(len(batch_data)):
            for k in range(batch_data[i].shape[0]):
                batch_data[i][k, ...] = np.dot(batch_data[i][k, ...].reshape((-1, 3)), rotation_matrix)   
        return batch_data
...
def rotate_perturbation_point_cloud(batch_data, angle_sigma=0.06, angle_clip=0.30):
    ...
    if not isinstance(batch_data, list):
        ...
        for i in range(len(batch_data)):
            for k in range(batch_data[i].shape[0]):
                batch_data[i][k, ...] = np.dot(batch_data[i][k, ...].reshape((-1, 3)), R)   
        return batch_data
...
class MyAugment:
    ...
    def augment(self, batch_data):
        is_list = isinstance(batch_data, list)
        
        if np.random.random() > 0.5:
            batch_data = rotate_perturbation_point_cloud(batch_data, angle_sigma=0.2, angle_clip=0.50)  # 0.06  0.3
        ...
        if not is_list:
            ...
            if np.random.random() > 0.5:
                batch_data[:-1] = shift_point_cloud(batch_data[:-1], 0.05)  # 法向量不需要增强
            if np.random.random() > 0.5:
                batch_data[:-1] = random_scale_point_cloud(batch_data[:-1])
        return batch_data
